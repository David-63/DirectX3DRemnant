...
#define PI 3.1415926535f
		...
		for (int lat = 0; lat <= latitudeSegments; ++lat) {
			for (int lon = 0; lon <= longitudeSegments; ++lon) {
				...
				Vec3 position = normal * radius + Vec3(0, height / 2, 0); // 상단 반구
				Vec2 uv = Vec2((float)lon / longitudeSegments, 1.f - (float)lat / latitudeSegments);
				Vec4 color = Vec4(1.0f, 1.0f, 1.0f, 1.0f); // 흰색
				...
				// 하단 반구 (y 좌표와 uv.v 좌표 반전)
				position = normal * radius + Vec3(0, -height / 2, 0);
				uv.y = 1.0f - uv.y;
		...
		for (int lon = 0; lon <= longitudeSegments; ++lon) {
			...
			Vec3 normal = Vec3(x, 0, z).Normalize();
			Vec3 topPosition = Vec3(x * radius, height / 2, z * radius);
			Vec3 bottomPosition = Vec3(x * radius, -height / 2, z * radius);
			Vec2 uvTop = Vec2((float)lon / longitudeSegments, 0.f);
			Vec2 uvBottom = Vec2((float)lon / longitudeSegments, 1.f);

			v.vNormal = normal;
			v.vPos = topPosition;
			v.vUV = uvTop;
			vecVtx.push_back(v); // 상단 원통

			v.vPos = bottomPosition;
			v.vUV = uvBottom;
			vecVtx.push_back(v); // 하단 원통
		}
		...
		for (int lat = 0; lat < latitudeSegments; ++lat) {
			for (int lon = 0; lon < longitudeSegments; ++lon) {
				...
				vecIdx.push_back(current);
				vecIdx.push_back(current + 1);
				vecIdx.push_back(next + 1);

				vecIdx.push_back(current);
				vecIdx.push_back(next + 1);
				vecIdx.push_back(next);
			}
		}

		// 원통 idx
		int baseIndex = latitudeSegments * (longitudeSegments + 1); // 원통 시작 인덱스
		for (int lon = 0; lon < longitudeSegments; ++lon) {
			...
			vecIdx.push_back(current);
			vecIdx.push_back(current + 1);
			vecIdx.push_back(next + longitudeSegments + 1);

			vecIdx.push_back(current);
			vecIdx.push_back(next + longitudeSegments + 1);
			vecIdx.push_back(next + longitudeSegments);
		}

		// 하단 반구
		baseIndex += (longitudeSegments + 1) * 2; // 하단 반구 시작 인덱스
		for (int lat = 0; lat < latitudeSegments; ++lat) {
			for (int lon = 0; lon < longitudeSegments; ++lon) {
				int current = baseIndex + lat * (longitudeSegments + 1) + lon;
				int next = current + longitudeSegments + 1;
...
// ============================
	...
	pShader = new CGraphicsShader;
	pShader->SetKey(L"Std3DUIShader");
...
void CResMgr::CreateDefaultMaterial()
{
	...
	pMtrl = new CMaterial(true);
	pMtrl->SetShader(FindRes<CGraphicsShader>(L"Std3DUIShader"));
	AddRes(L"Std3DUIMtrl", pMtrl);
