...
CWeightMapShader::CWeightMapShader(UINT _iGroupPerThreadX, UINT _iGroupPerThreadY, UINT _iGroupPerThreadZ)
	: m_pWeightMap(nullptr), m_pRaycastInfo(nullptr)
	, m_iBrushIdx(0), m_iWeightIdx(0)
...
{
	m_Const.arrInt[0] = static_cast<int>(m_iWidth);
	m_Const.arrInt[1] = static_cast<int>(m_iHeight);
